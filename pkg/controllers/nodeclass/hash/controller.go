...
func (c *Controller) Reconcile(ctx context.Context, nodeClass *v1alpha1.ClusterAPINodeClass) (reconcile.Result, error) {
	...
	if nodeClass.Annotations[v1alpha1.AnnotationClusterAPINodeClassHashVersion] != v1alpha1.ClusterAPINodeClassHashVersion {
		if err := c.updateNodeClaimHash(ctx, nodeClass); err != nil {
			return reconcile.Result{}, err
		}
	}
	nodeClass.Annotations = lo.Assign(nodeClass.Annotations, map[string]string{
		v1alpha1.AnnotationClusterAPINodeClassHash:        nodeClass.Hash(),
		v1alpha1.AnnotationClusterAPINodeClassHashVersion: v1alpha1.ClusterAPINodeClassHashVersion,
	})
...
// Updating `ClusterAPINodeClass-hash-version` annotation inside the karpenter controller means a breaking change has been made to the hash calculation.
// `ClusterAPINodeClass-hash` annotation on the ClusterAPINodeClass will be updated, due to the breaking change, making the `ClusterAPINodeClass-hash` on the NodeClaim different from
// ClusterAPINodeClass. Since, we cannot rely on the `ClusterAPINodeClass-hash` on the NodeClaims, due to the breaking change, we will need to re-calculate the hash and update the annotation.
...
func (c *Controller) updateNodeClaimHash(ctx context.Context, nodeClass *v1alpha1.ClusterAPINodeClass) error {
	...
	for i := range ncList.Items {
		...
		if nc.Annotations[v1alpha1.AnnotationClusterAPINodeClassHashVersion] != v1alpha1.ClusterAPINodeClassHashVersion {
